...
class gated_resnet(nn.Module):
    def __init__(
        ...
        # NEW: Conditional fusion module.
        # If conditional_channels > 0, create a mapping from the conditional input to num_filters.
        self.conditional_channels = conditional_channels
        if self.conditional_channels > 0:
            self.nin_cond = nin(self.conditional_channels, num_filters)
    ...
    def forward(self, og_x, a=None, cond=None):
        ...
        # NEW: Incorporate conditional fusion within the block.
        if cond is not None and self.conditional_channels > 0:
            x += self.nin_cond(self.nonlinearity(cond))
