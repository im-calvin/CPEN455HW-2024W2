...
def train_or_test(
    ...
    for batch_idx, (model_input, labels) in enumerate(tqdm(data_loader)):
        ...
        nums = [
            my_bidict.get(label) for label in labels
        ]  # TODO this doesn't work during validation for some reason (labels is None)
        numeric_labels = torch.tensor(nums, dtype=torch.long).to(device)
